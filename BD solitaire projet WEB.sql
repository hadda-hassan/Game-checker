...
x_case NUMBER(3),
y_case NUMBER(3),
constraint pk_case PRIMARY KEY (Idcase)
);
------------------------------------------------------------------------------------------------
--TABLE Coup
------------------------------------------------------------------------------------------------
...
CONSTRAINT fk_case_d foreign key (depart) references CASES(idcase),
CONSTRAINT fk_case_a foreign key (arrivee) references CASES(idcase),

);
------------------------------------------------------------------------------------------------
--TABLE Construire_niveau # pour selectionner les billes correspendant a chaque niveau
------------------------------------------------------------------------------------------------
-- premier enmplacement de chaque bille dans un niveau doit liée a  les idcase qui sont rempler
-- en etat initiale idbille=idcase( depare de jeu)
...
------------------------------------------------------------------------------------------------
 ...
 --- le maximum de l'idbille est 32 en fonction de coordonnées dans le plateau solitaire pliennes
 -- alors en rempler de 1 jusqu'à 49 
...
------------------------------------------------------------------------------------------------
---  remplir tables des CASES
------------------------------------------------------------------------------------------------

 --- le maximum nombres des cases est 7X7 alors 49 cases parmi ces cases on as 16 qui sont injoigniables
 -- ce que nous permet d'utiliser juste 33 cases dans chaque plateaux
 DECLARE 
   i number(2); 
BEGIN  
   FOR i IN 1..7 LOOP 
     insert INTO cases values (i,1,i);-- premier ligne
   END loop;
   FOR i IN 1..7 LOOP 
     insert INTO cases values (i+7,2,i);-- deuxieme colonne
   END loop; 
    FOR i IN 1..7 LOOP 
     insert INTO cases values (i+14,3,i);-- 3 eme colonne
   END loop; 
    FOR i IN 1..7 LOOP 
     insert INTO cases values (i+21,4,i);--  
   END loop; 
    FOR i IN 1..7 LOOP 
     insert INTO cases values (i+28,5,i);--  
   END loop; 
    FOR i IN 1..7 LOOP 
     insert INTO cases values (i+35,6,i);-- 
   END loop; 
    FOR i IN 1..7 LOOP 
     insert INTO cases values (i+42,7,i);-- 7 eme colonne
   END loop; 
END; 
/
...
-- cette procedure on la pas utiliser puisque on as gérer la virification de mot de passe par flask
------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------
-- ICI on as pas gérer les exeception de data notefound puisque on as besoin juste d'enregitrer
CREATE OR REPLACE PROCEDURE enregistrer_coup (p_bille bille.IdBille%type, p_partie partie.IdPartie%type, 
	em_depart cases.Idcase%type, em_arrivee cases.Idcase%type) IS  
...
------------------------------------------------------------------------------------------------
--------- procedure rejouer_partie  # revoir mes partie fini avec id partie 
------------------------------------------------------------------------------------------------
-- voir la sumilation des coups dans mes partie finis

create or replace Procedure rejouer_partie(p_idPartie in Partie.IdPartie%type, cur_coup out SYS_REFCURSOR) IS
...
-----------------------------------------------------------------------------------------------
create or replace view classement_highscore as
select identifiant,Scoreperso
from JOUEUR order by Scoreperso desc;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON <niveau> TO gdn2950a;
GRANT SELECT, INSERT, UPDATE, DELETE ON <JOUEUR> TO gdn2950a;
GRANT SELECT, INSERT, UPDATE, DELETE ON <partie> TO gdn2950a;
GRANT SELECT, INSERT, UPDATE, DELETE ON <bille> TO gdn2950a;
GRANT SELECT, INSERT, UPDATE, DELETE ON <Construire_niveau> TO gdn2950a;
GRANT SELECT, INSERT, UPDATE, DELETE ON <cases> TO gdn2950a;
GRANT SELECT, INSERT, UPDATE, DELETE ON <coup> TO gdn2950a;
